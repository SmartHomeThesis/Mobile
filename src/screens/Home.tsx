...
import {getAllDevice, removeAllStateDevice} from "../redux/reducers/deviceSlice";
...
const Home = ({navigation}: { navigation: any }) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            console.log("livingRoom", state.livingRoom);
            console.log("parkingGarage", state.parkingGarage);
        }
        fetchData().catch((error) => {
            console.error("fetchData error", error);
        })
        // return () =>{
        //     console.log("handel clean up");
                ...
                <View style={styles.boxContainer}>
                    {LivingRoom.map((item, index) => (
                        <ListDevice
                            id={item.id}
                            name={item.name}
                            status={state?.livingRoom[0]?.status === "online" }
                            image={item.image}
                            key={index}
                            onPress={() => {
                ...
                <View style={styles.boxContainer}>
                    {ParkingGarage.map((item, index) => (
                        <ListDevice
                            id={item.id}
                            name={item.name}
                            status={state?.parkingGarage[0]?.status === "online"}
                            image={item.image}
                            key={index}
                            onPress={() => {
                                navigation.navigate("DetailDevice", {
